	...
	if(objJSONParser)
	{
		...
		for(string::iterator strItr = strJSON.begin(); strItr != strJSON.end(); ++ strItr)
		{
			...
			case JSONAutoMataState_Dic_Begin:
				...
				vecConstructor.push_back(JSONParserWrapper(JSONParserWrapperType_Map, tmpMap));
				break;
			case JSONAutoMataState_Dic_End:
				...
				if(vecConstructor.size() > 2) // 数量满足
				{
					...
					if(refNewWrapper.eType == JSONParserWrapperType_String && 
						refNewSecondWrapper.eType == JSONParserWrapperType_Map)
					{
			...
			case JSONAutoMataState_Dic_Value_End:
				...
				if(vecConstructor.size() > 1 && 
					...
					refNewSecondWrapper.eType == JSONParserWrapperType_Map) // 上一层是字典，且有对应key在等待中
				{
			...
			case JSONAutoMataState_String_End:
				...
				if(refWrapper.eType == JSONParserWrapperType_StringBuffer)
				{
					...
					else if(refWrapper.eType == JSONParserWrapperType_Vector) // string是数组的元素，再放回去
					{
						string* strValue = new string(strResult);
			...
			case JSONAutoMataState_Arr_Begin:
				...
				vecConstructor.push_back(JSONParserWrapper(JSONParserWrapperType_Vector, pVecArr));
				break;
			case JSONAutoMataState_Arr_Elem_Seg:
				...
				if(refNewWrapper.eType == JSONParserWrapperType_Vector)
				{
			...
			case JSONAutoMataState_Arr_End:
				...
				if(ePriorState == JSONAutoMataState_Arr_Begin) // 空数组的情况
				{
					...
					refWrapper = vecConstructor.back();
					if(refWrapper.eType == JSONParserWrapperType_Vector)
					{
						// 检查value的情况
						if(vecConstructor.size() > 1)
						{
					...
					else if (refWrapper.eType == JSONParserWrapperType_Bool || refWrapper.eType == JSONParserWrapperType_Int || refWrapper.eType == JSONParserWrapperType_Double || refWrapper.eType == JSONParserWrapperType_Null)
					{
						// 需要处理基本类型的情况
						ePriorState = JSONAutoMataState_Arr_End;
						// 此时前面还有一个元素等待入列表
...
map<string, JSONParserWrapper>* JSONParser::wrapperParse(string strJSON)
{
	JSONParserWrapper refWrapper;
	JSONParserWrapper refNewWrapper;
	JSONParserWrapper refNewSecondWrapper;

	map<string, JSONParserWrapper>* tmpMap = NULL;
	void* pNewBack = NULL;
	stringstream* ssBuffer = NULL;
	void* pCurrentBack = NULL;
	vector<JSONParserWrapper>* pVecNewBack = NULL;
	void* pNewSecondBack = NULL;
	char* pcPlaceHolder = NULL;
	void* pBack = NULL;
	vector<JSONParserWrapper>* pVecArr = NULL;
	...
	if (objJSONParser)
	{
		...
		for (string::iterator strItr = strJSON.begin(); strItr != strJSON.end(); ++strItr)
		{
			...
			case JSONAutoMataState_Dic_Begin:
				ePriorState = JSONAutoMataState_Dic_Begin;
				tmpMap = new map<string, JSONParserWrapper>();
				vecConstructor.push_back(JSONParserWrapper(JSONParserWrapperType_Map, tmpMap));
				break;
			case JSONAutoMataState_Dic_End:
				if (vecConstructor.size() == 0)
				...
				if (vecConstructor.size() > 2) // 数量满足
				{
					...
					if (refNewWrapper.eType == JSONParserWrapperType_String &&
						refNewSecondWrapper.eType == JSONParserWrapperType_Map)
					{
						map<string, JSONParserWrapper>* pMap = (map<string, JSONParserWrapper>*)refNewSecondWrapper.pData;
						string strKey = string((*((string*)refNewWrapper.pData)));
						delete ((string*)refNewWrapper.pData);

						pMap->insert(pair<string, JSONParserWrapper>(strKey, refWrapper));

						vecConstructor.pop_back();
						vecConstructor.pop_back();

						refWrapper = vecConstructor.back(); // 这里似乎没用，一会儿检查一下删掉
					}
				}
				...
				{
					refWrapper = vecConstructor.back();
					tmpMap = (map<string, JSONParserWrapper>*)refWrapper.pData;
					return tmpMap;
				}
				else if (vecConstructor.size() > 1)
				{
			...
			case JSONAutoMataState_Dic_Value_End:
				...
				if (refWrapper.eType == JSONParserWrapperType_Bool ||
					...
					refWrapper.eType == JSONParserWrapperType_Null)
				{
					vecConstructor.pop_back(); // 弹出的是PlaceHolder
				}
				...
				if (vecConstructor.size() > 1)
				{
					refNewSecondWrapper = vecConstructor.at(vecConstructor.size() - 2); // map之前还有key没用pop，所以是减2
				}
				if (vecConstructor.size() > 1 &&
					refNewWrapper.eType == JSONParserWrapperType_String &&
					refNewSecondWrapper.eType == JSONParserWrapperType_Map) // 上一层是字典，且有对应key在等待中
				{
					...
					vecConstructor.pop_back(); // 原来保存的key不用再留着了
					delete ((string*)refNewWrapper.pData);
					// 加入到等待的map中
					map<string, JSONParserWrapper>* pMapNewBack = (map<string, JSONParserWrapper>*)refNewSecondWrapper.pData;
					pMapNewBack->insert(pair<string, JSONParserWrapper>(strKey, refWrapper));
				}
				else // 存在错误
				{
					return NULL;
			...
			case JSONAutoMataState_Arr_Begin:
				...
				ePriorState = JSONAutoMataState_Arr_Begin;
				pVecArr = new vector<JSONParserWrapper>();
				vecConstructor.push_back(JSONParserWrapper(JSONParserWrapperType_Vector, pVecArr));
				break;
			case JSONAutoMataState_Arr_Elem_Seg:
				if (vecConstructor.size() == 0)
				...
				if (refNewWrapper.eType == JSONParserWrapperType_Vector)
				{
					pVecArr = (vector<JSONParserWrapper>*)refNewWrapper.pData;
					pVecArr->push_back(refWrapper);
				}
				else // 有问题
				{
					return NULL;
...
void* JSONParser::mapSafeAccess(map<string, JSONParserWrapper>* mpJSON, string strKey)
{
	void* result = NULL;
	if (mpJSON && mpJSON->size() > 0)
	{
		map<string, JSONParserWrapper>::iterator mapItr = mpJSON->find(strKey);
		if (mapItr != mpJSON->end())
		{
			result = (mapItr->second).pData;
		}
	}
	return result;
}

void* JSONParser::mapSafeAccess(map<string, JSONParserWrapper>* mpJSON, string strKey, JSONParserWraperType dataType)
{
	void* result = NULL;
	if (mpJSON && mpJSON->size() > 0)
	{
		...
		if (mapItr != mpJSON->end())
		{
			JSONParserWrapper tmpWrapper = mapItr->second;
			if (tmpWrapper.eType == dataType)
			{
				result = tmpWrapper.pData;
			}
		}
	}
	return result;
...
int JSONParser::mapDelete(map<string, JSONParserWrapper>* mpJSON)
{
	for (map<string, JSONParserWrapper>::iterator mapItr = mpJSON->begin(); mapItr != mpJSON->end(); ++mapItr)
	{
		int nRet = 0;
		JSONParserWrapper tmpWrapper = mapItr->second;
		switch (tmpWrapper.eType)
		{
		case JSONParserWrapperType_Map:
			nRet = mapDelete((map<string, JSONParserWrapper>*)tmpWrapper.pData);
			if (nRet != 0)
			{
				return nRet;
			}
			break;
		case JSONParserWrapperType_Vector:
			nRet = vecDelete((vector<JSONParserWrapper>*)tmpWrapper.pData);
			if (nRet != 0)
			{
				return nRet;
			}
...
int JSONParser::vecDelete(vector<JSONParserWrapper>* vecJSON)
{
	for (vector<JSONParserWrapper>::iterator vecItr = vecJSON->begin(); vecItr != vecJSON->end(); ++vecItr)
	{
		int nRet = 0;
		JSONParserWrapper tmpWrapper = (*vecItr);
		switch (tmpWrapper.eType)
		{
		case JSONParserWrapperType_Map:
			nRet = mapDelete((map<string, JSONParserWrapper>*)tmpWrapper.pData);
